...
.MACRO	SYNCHK	 Q 
	...
	JSR	SYNCHR
.ENDMACRO
;NOTICE: do not use this dt definition
;DEFINE	DT(Q),<
...
.MACRO	SKIP1	
.BYTE $24
.ENDMACRO	;BIT ZERO PAGE TRICK.
.MACRO	SKIP2	
.BYTE $2C
.ENDMACRO	;BIT ABS TRICK.
...
INTEGR .SET  CHARAC			;A ONE-BYTE INTEGER FROM "QINT".
...
KIMY .SET 	DIMFLG			;PLACE TO PRESERVE Y DURING OUT.
...
GARBFL .SET  DORES			;WHETHER TO DO GARBAGE COLLECTION.
...
RESMO:	BLOCK	1
RESLO:	BLOCK	1
ADDEND .SET  RESMO			;TEMPORARY USED BY "UMULT".
...
OLDLIN: BLOCK	2		;OLD LINE NUMBER (SETUP BY ^C,"STOP"
				;OR "END" IN A PROGRAM).
POKER .SET 	LINNUM			;SET UP LOCATION USED BY POKE.
...
FDECPT .SET  VARPNT			;POINTER INTO POWER OF TENS OF "FOUT".
FORPNT: BLOCK	2		;A VARIABLE'S POINTER FOR "FOR" LOOPS
				;AND "LET" STATEMENTS.
LSTPNT .SET  FORPNT			;PNTR TO LIST STRING.
ANDMSK .SET  FORPNT			;THE MASK USED BY WAIT FOR ANDING.
EORMSK .SET  FORPNT+1		;THE MASK FOR EORING IN WAIT.
OPPTR:	BLOCK	2		;POINTER TO CURRENT OP'S ENTRY IN "OPTAB".
VARTXT .SET  OPPTR			;POINTER INTO LIST OF VARIABLES.
OPMASK: BLOCK	1		;MASK CREATED BY CURRENT OPERATOR.
DOMASK .SET TANSGN			;MASK IN USE BY RELATION OPERATIONS.
DEFPNT: BLOCK	2		;POINTER USED IN FUNCTION DEFINITION.
GRBPNT .SET  DEFPNT			;ANOTHER USED IN GARBAGE COLLECTION.
DSCPNT: BLOCK	2		;POINTER TO A STRING DESCRIPTOR.
.IF	ADDPRC <> 0
BLOCK	1
.ENDIF	;FOR TEMPF3.
;NOTICE: EXP is not correct
...
JMPER:	JMP	60000
SIZE .SET 	JMPER+1
OLDOV .SET 	JMPER+2			;THE OLD OVERFLOW.
TEMPF3 .SET  DEFPNT			;A THIRD FAC TEMPORARY (4 BYTES).
TEMPF1:
.IF	ADDPRC <> 0
.BYTE 0
...
ARYPNT .SET  HIGHDS			;A POINTER USED IN ARRAY BUILDING.
GRBTOP .SET  LOWTR			;A POINTER USED IN GARBAGE COLLECTION.
DECCNT .SET  LOWDS			;NUMBER OF PLACES BEFORE DECIMAL POINT.
TENEXP .SET  LOWDS+1			;HAS A DPT BEEN INPUT?
DPTFLG .SET  LOWTR			;BASE TEN EXPONENT.
EXPSGN .SET  LOWTR+1			;SIGN OF BASE TEN EXPONENT.
...
DEGREE .SET  SGNFLG			;A COUNT USED BY POLYNOMIALS.
DSCTMP .SET  FAC			;THIS IS WHERE TEMP DESCS ARE BUILT.
INDICE .SET  FACMO			;INDICE IS SET UP HERE BY "QINT".
...
STRNG1 .SET  ARISGN			;POINTER TO A STRING OR DESCRIPTOR.

FBUFPT: BLOCK	2		;POINTER INTO FBUFFR USED BY FOUT.
BUFPTR .SET  FBUFPT			;POINTER TO BUF USED BY "CRUNCH".
STRNG2 .SET  FBUFPT			;POINTER TO STRING OR DESC.
POLYPT .SET  FBUFPT			;POINTER INTO POLYNOMIAL COEFFICIENTS.
CURTOL .SET  FBUFPT			;ABSOLUTE LINEAR INDEX IS FORMED HERE.
;PAGE
;SUBTTL	RAM CODE.
...
.ENDIF		;CLOSE ALL OPEN FILES.
	...
	STWD	STREND		;ARRAYS.
FLOAD:	JSR	RESTORE		;RESTORE DATA.
;
...
READ:	LDXY	DATPTR		;GET LAST DATA LOCATION.
	...
	.BYTE	$A9	;LDI TYA TO MAKE IT ZERO.
.IF	BUFPAG = 0
...
;NOTICE: use .BYTE to replace EXP
;N32768: EXP	144,128,0,0	;-32768.
N32768: .BYTE	144,128,0,0
...
QINT:	LDA	FACEXP
	...
	SEC
	SBC #	$00*ADDPRC+$98	;GET NUMBER OF PLACES TO SHIFT.
	BIT	FACSGN
	BPL	QISHFT
	TAX
	LDA #	$FF
...
;NOTICE: do not use
;IF2,<
;	PURGE	A,X,Y;	PURGE	A,X,Y;IFNDEF	START,<START==0>;	END	$Z+START